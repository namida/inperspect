...
void KinectOpenCV::KinectThread(void) {
	...
	if( (freenect_set_video_format(this->fdev, FREENECT_VIDEO_RGB)) != 0 ) {
		throw std::runtime_error("Cannot set video format");
	}

	if( (freenect_set_depth_format(this->fdev, FREENECT_DEPTH_11BIT)) != 0 ) {
...
void KinectOpenCV::SetLED(int led_mode) {
	if( led_mode < 0 || led_mode > 6 ) {
		...
		freenect_set_led(this->fdev, LED_BLINK_YELLOW);
	}
	else if (led_mode == '5') {
